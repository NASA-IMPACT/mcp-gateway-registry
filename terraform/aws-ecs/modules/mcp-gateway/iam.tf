    ...
    Statement = [
      {
        ...
        Resource = concat(
          [
            ...
            # Referenced unconditionally by the registry task definition
            # (ecs-services.tf), but was missing here, so the task could not
            # start: ResourceInitializationError on GetSecretValue.
